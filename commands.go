		...
		Handler: func(s *discordgo.Session, i *discordgo.InteractionCreate) error {
			...
			for page < 5 {
				...
				for _, m := range msgs {
					...
					match := messageRegex.FindStringSubmatch(m.Content)

					var amount int
					if len(match) >= 2 {
						amount, err = strconv.Atoi(strings.Replace(match[1], ",", "", -1))
						if err != nil {
							slog.Warn("invalid amount", slog.String("content", m.Content))
							continue
						}
					} else {
						continue
					...
					slog.Info("found message", slog.Int("amount", amount), slog.String("label", label), slog.Any("mentions", m.Mentions))
